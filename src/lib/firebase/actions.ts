...
export async function actionCheckIn(
  ...
  if (currentUser.access_level === FSUserAccessLevel.Operator) {
  ...
  const newRecord = {
    ...
    status: 'CHECKED_IN',
    comments:
      `${comments}\n\n(Check-in completed by ${currentUser.display_name?.toUpperCase()} on ${timestamp.toLocaleDateString()} at ${timestamp.toLocaleTimeString()} hours)`.trim(),
...
export async function actionRefuseEntry(
  ...
  const newRecord = {
    ...
    status: 'REFUSED',
    comments:
      `${comments}\n\n(Entry refused by ${currentUser.display_name?.toUpperCase()} on ${timestamp.toLocaleDateString()} at ${timestamp.toLocaleTimeString()} hours)`.trim(),
    updated_at: serverTimestamp(),
  };
...
export async function actionSetConflict(
  ...
  const newRecord = {
    ...
    status: 'CONFLICT',
    comments:
      `${comments}\n\n(Handover initiated by ${currentUser.display_name?.toUpperCase()} on ${timestamp.toLocaleDateString()} at ${timestamp.toLocaleTimeString()} hours)`.trim(),
    updated_at: serverTimestamp(),
  };
